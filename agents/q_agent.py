...
from networks.dqn import DQN, ReplayMemory
...
class QAgent:
    ...
    def get_state(self, env, player):
        ...
        To the state obtained from self.get_state_just_hand() a vector of length
        40 appended. Each entry is associated with one card in the deck. At each
        step of the episode the agent observes the played cards and the card in
        his hand and set the corresponding entry in the vector to 1.
        """
        state = np.zeros(25)
        value_offset = 1
        seed_offset = 3
    ...
    def load(self, path):
        "Loads policy network's state disctionary from path"
